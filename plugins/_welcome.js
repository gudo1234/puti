...
let handler = async (m, { conn, __dirname }) => {
   ...
   *
   * 1. "123456789@s.whatsapp.net"
   *
   * 2. "123456789@lid"
   *
   * 3. '{"id":"123456789@lid","phoneNumber":"504...@s.whatsapp.net"}'
   *
   * 4. { id: '...', phoneNumber: '...' }
   */
  ...
  /*
   * Si todavía tenemos un LID, intentamos obtener
   * el JID real desde los participantes del grupo.
   */
  ...
  /*
   * Esperar a que Baileys vuelva a estar conectado.
   */
  ...
  /*
   * Envío protegido con reintentos.
   */
  ...
  try {
    const pp = await conn
      ...
      .catch(() => global.icono || null)

    if (pp) {
      const response = await fetch(pp)

      if (response.ok) {
        im = await response.buffer()
      }
    }
  } catch {
    im = null
  ...
  try {
    switch (formatoElegido) {
      ...
      case 'stiker': {
        try {
          ...
          /*
           * Si falla el sticker, intentamos texto.
           * Si la conexión también está cerrada,
           * safeSendMessage lo manejará.
           */
      ...
      case 'interactivo': {
        ...
        /*
         * relayMessage también tiene reintento,
         * porque puede ocurrir que la conexión se cierre
         * justo después de waitForConnection().
         */
        let enviado = false
    ...
    /*
     * No usamos conn.reply() aquí.
     *
     * Si el error es "Connection Closed",
     * mandar otro mensaje produciría otro error.
     */
    console.error(
      '[WELCOME] Error:',
      e?.message || e
