...
let handler = async (m, { conn, __dirname }) => {
  ...
  let ppResponse = await fetch(pp)
  let im = Buffer.from(await ppResponse.arrayBuffer())
  ...
  const thumbLocal = fs.existsSync(imgPath)
    ? fs.readFileSync(imgPath)
    : null

  const thumbResized = thumbLocal
    ? await sharp(thumbLocal)
        .resize(300, 100, { fit: 'cover' })
        .jpeg()
        .toBuffer()
    : null

  const iconoResponse = await fetch(icono)
  const iconoOriginal = Buffer.from(await iconoResponse.arrayBuffer())

  const rcanalThumbnail = await sharp(iconoOriginal)
    .resize(640, 640, {
      fit: 'cover',
      position: 'centre'
    })
  ...
  const stikerBienvenida = await sticker(
    imagen8,
    false,
    global.packname,
    global.author
  ...
  const stikerDespedida = await sticker(
    imagen7,
    false,
    global.packname,
    global.author
        ...
        await conn.sendMessage(
          m.chat,
          {
            video: {
              url: isWelcome
                ? gifsBienvenida[
                    Math.floor(Math.random() * gifsBienvenida.length)
                  ]
                : gifDespedida
            },
            gifPlayback: true,
          ...
          nativeFlowMessage: {
            buttons: [
              ...
              },
              {
                name: 'cta_copy',
                buttonParamsJson:
                  '{"display_text":"Copiar Código","id":"123456789","copy_code":"🙇🏿‍♂️ Negro de mierd :v"}'
              },
              {
                name: 'cta_url',
                buttonParamsJson: `{
