...
global.mundo = async function (m, conn) {
    try {
        ...
        try {
            const intl = new Intl.DisplayNames(["es"], {
                type: "region"
            })
...
const responseIcono = await fetch(icono)
const iconoBuffer = Buffer.from(await responseIcono.arrayBuffer())

const rcanalThumbnail = await sharp(iconoBuffer)
    .resize(640, 640, {
        fit: "cover",
        position: "centre"
    })
