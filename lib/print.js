...
export default async function (m, conn = { user: {} }) {
  ...
  console.log(
    chalk.yellow.bold('ミ★ 𝘌𝘯𝘪𝘨𝘮𝘢-𝘉𝘰𝘵 ★彡\n'),
    chalk.blue('Mensaje'),
    chalk.blue('de'),
    chalk.blue(`${m.sender} ${_name}`)
    //chalk.white(`: ${m.text || ''}`)
  )
  ...
  if (typeof m.text === 'string' && m.text) {
    ...
    if (m.mentionedJid) {
      const resolveMentionLabel = async (jid) => {
        try {
          const name = await conn.getName(jid)
          if (name) return name
        } catch {}
        let resolvedJid = jid
        if (m.isGroup && typeof jid === 'string' && jid.endsWith('@lid')) {
          try {
            const metadata = await conn.groupMetadata(m.chat)
            const match = metadata?.participants?.find(p => p.id === jid && p.jid)
            if (match?.jid) resolvedJid = match.jid
          } catch {}
        }
        return resolvedJid || jid
      }
      for (let user of m.mentionedJid) {
        const label = await resolveMentionLabel(user)
        const display = label || user
        log = log.replace('@' + user.split`@`[0], chalk.blueBright('@' + display))
      }
    }
