...
// Baileys can expose its API through the ESM namespace, the default export,
// or a mixture of both depending on the installed build. Resolve each value
// from both places so the serializer does not break at startup.
const _makeWaSocket = baileysModule.makeWASocket || baileysDefault.makeWASocket
const makeWALegacySocket = baileysModule.makeWALegacySocket || baileysDefault.makeWALegacySocket
const proto = baileysModule.proto || baileysDefault.proto
const downloadContentFromMessage = baileysModule.downloadContentFromMessage || baileysDefault.downloadContentFromMessage
const jidDecode = baileysModule.jidDecode || baileysDefault.jidDecode
const areJidsSameUser = baileysModule.areJidsSameUser || baileysDefault.areJidsSameUser
const generateForwardMessageContent = baileysModule.generateForwardMessageContent || baileysDefault.generateForwardMessageContent
const generateWAMessageFromContent = baileysModule.generateWAMessageFromContent || baileysDefault.generateWAMessageFromContent
const WAMessageStubType = baileysModule.WAMessageStubType || baileysDefault.WAMessageStubType
const extractMessageContent = baileysModule.extractMessageContent || baileysDefault.extractMessageContent
const prepareWAMessageMedia = baileysModule.prepareWAMessageMedia || baileysDefault.prepareWAMessageMedia
const WA_DEFAULT_EPHEMERAL = baileysModule.WA_DEFAULT_EPHEMERAL || baileysDefault.WA_DEFAULT_EPHEMERAL
...
export function serialize() {
    if (!proto?.WebMessageInfo?.prototype) {
        throw new Error('Baileys no expuso proto.WebMessageInfo. Verifica la versión de @whiskeysockets/baileys instalada.')
    }
